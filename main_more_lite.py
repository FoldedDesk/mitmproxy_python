...
class AdvancedLogger:
    ...
    def request(self, flow: http.HTTPFlow) -> None:
        ...
        # 只记录特定Content-Type的流量到完整日志
        self._save_complete_log(flow, is_request=True)

        # 仅当设置了filter时才尝试记录到过滤日志
    ...
    def _save_complete_log(self, flow, is_request: bool):
        """记录所有流量到完整日志，但只记录特定Content-Type"""
        # 获取Content-Type
        content_type = ""
        if is_request:
    ...
    def _save_filtered_log(self, flow, is_request: bool):
        """仅当设置了filter时记录匹配流量到过滤日志，且只记录特定Content-Type"""
        # 获取Content-Type
        content_type = ""
        if is_request:
            content_type = flow.request.headers.get("Content-Type", "")
